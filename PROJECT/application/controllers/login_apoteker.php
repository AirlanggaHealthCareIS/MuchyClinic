...
class Login_apoteker extends CI_Controller {

	public function Login_apoteker () {
		parent::__construct();
		$this->login_title = "apoteker";
		$this->login_url_validasi = base_url()."login_apoteker/validasi";
	...
	public function validasi()
	{
		...
		if (($username==null || $username=="") && ($password==null || $password=="")) {
			redirect(base_url().'Login_apoteker?error=null');
		} 
		
		if ($username==null || $username=="") {
			redirect(base_url().'Login_apoteker?error=nullusername');
		} 

		if ($password==null || $password=="") {
			redirect(base_url().'Login_apoteker?error=nullpassword');
		} 
		
		if (preg_match('/[^a-z0-9]/',$username)) {
			redirect(base_url().'Login_apoteker?error=symbol');	
		}
